...
export interface SourceCodeProvider {
   ...
   * опционально фильтруя по изменениям после указанного коммита
   */
  getChangedFiles(prId: number | string, sinceCommit?: string): Promise<CodeFile[]>;

  /**
   * Получить текущий HEAD коммит PR/MR
   */
  getCurrentCommit(prId: number | string): Promise<string>;
}
...
export interface CodeFile {
  ...
  content: string;
  patch?: string;
  commits?: string[]; // Список коммитов, затрагивающих файл
}
...
export class GitHubAdapter implements CodeReviewPlatform {
   ...
   */
  async getChangedFiles(prNumber: number, sinceCommit?: string): Promise<CodeFile[]> {
    ...
    if (sinceCommit) {
      try {
        ...
        const { data: commits } = await withRetry(() => this.octokit.repos.compareCommits({
          owner: this.owner,
          repo: this.repo,
          base: sinceCommit,
          head: headCommit,
        ...
        for (const commit of commits.commits) {
          ...
          try {
            const { data: commitData } = await withRetry(() => this.octokit.repos.getCommit({
              owner: this.owner,
              repo: this.repo,
              ref: commit.sha,
            }));
   ...
   */
  async getCurrentCommit(prNumber: number): Promise<string> {
...
export class GitLabAdapter implements CodeReviewPlatform {
   ...
   */
  async getChangedFiles(mergeRequestIid: string | number, sinceCommit?: string): Promise<CodeFile[]> {
    ...
    if (sinceCommit) {
      try {
        ...
        const { commits } = await this.makeRequest<{ commits: Array<{ id: string, short_id: string }> }>(commitsEndpoint);
        ...
        if (lastCommitIndex === -1) {
          ...
          for (const commit of newCommits) {
            try {
              ...
              const { diffs } = await this.makeRequest<{ diffs: Array<{ new_path: string }> }>(commitEndpoint);
   ...
   */
  async getCurrentCommit(mergeRequestIid: string | number): Promise<string> {
    const endpoint = `/projects/${this.projectId}/merge_requests/${mergeRequestIid}`;
    const mr = await this.makeRequest<{ sha: string }>(endpoint);
    return mr.sha;
  }
