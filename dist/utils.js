...
exports.generateCommentsForChanges = generateCommentsForChanges;
exports.fetchFromDeepSeekAPI = fetchFromDeepSeekAPI;
exports.findSimilarCodeInDiff = findSimilarCodeInDiff;
    ...
    try {
      ...
      Ответ должен быть в формате JSON со следующей структурой:
      {
        ...
        const response = await withRetry(async () => {
            const fetchPromise = (0, node_fetch_1.default)(deepseekApiUrl, {
                method: 'POST',
                headers: {
        ...
        for (const issue of analysis.issues) {
            ...
            const match = findMostSimilarLine(issue.code, fileLines, searchRangeStart, searchRangeEnd);
...
async function generateCommentsForChanges(filePath, codeChanges, fileContent, repo) {
    ...
    const messages = [
        {
            role: "system", content: `You are a code review assistant. Your task is to review code changes and provide helpful comments.
      ...
      Be concise and specific. Don't comment on trivial issues.`
        ...
        {
            role: "user", content: `Review the following code changes in file ${filePath} from repository ${repo}.
...
async function fetchFromDeepSeekAPI(payload, endpoint = 'https://api.deepseek.com/v1/chat/completions', retries = 3, initialDelay = 1000) {
...
function findSimilarCodeInDiff(file, code, threshold = 0.7) {
...
async function generateReviewComments(diffData, getFileContent, repo) {
    ...
    for (const [filePath, fileChanges] of Object.entries(diffData)) {
        ...
        try {
            ...
            const aiComments = await generateCommentsForChanges(filePath, addedCodeBlocks.join('\n\n'), fileContent, repo);
            ...
            for (const aiComment of aiComments) {
                ...
                const match = findSimilarCodeInDiff({ changes, content: fileContent || undefined }, aiComment.code, 0.7 // Порог похожести
                );
                if (match) {
                    comments.push({
                        path: filePath,
